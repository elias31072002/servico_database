...
app.get('/listProject', listProjectHandler);
app.post('/add', addProjectHandler);

app.get('/listUser', listUserHandler);
...
/* Tratador de listagem */
async function listUserHandler(req, res){ 
    console.log("Requisição de listagem recebida."); //Para debug somente.
    let usuarios = await service.listUsers();  
    let json_usr_list = JSON.stringify(usuarios);
    res.setHeader('Content-Type', 'application/json');
    res.end(json_usr_list);     
}
